	...
	$.fn.advancedtable = function(custom) {
		...
		function redrawTable() {
			...
			if(tableRows > settings.rowsPerPage){
				...
				if(true){
					blockNavigation += '<a id="prev">Prev</a> <a id="next">Next</a> <ul class="PageNav_ul"> ';
...
function tableNavLiExtention(){
	...
	var total_linum = $(".PageNav_ul li").length;
	if(total_linum < 10){
		$("#next").hide();
		$("#prev").hide();
	}
...
var visible = "";
//$('.PageNav_ul li').hide().filter(visible).show();

var itemsNumber = 10;
var min = 0;
var max = itemsNumber;

function pagination_a(action) {

    var totalItems = $(".PageNav_ul li" + visible).length;
    ...
    $(".PageNav_ul li").hide();
    $(".PageNav_ul li" + visible).slice(min, max).show();
